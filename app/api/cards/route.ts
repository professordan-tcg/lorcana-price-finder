...
// Convert USD → GBP using an env var rate (e.g., 0.78 means $1 = £0.78)
function usdToGbp(usd) {
  const rate = Number(process.env.JUSTTCG_GBP_PER_USD || "");
  if (!rate || !isFinite(rate)) return usd; // fallback: leave USD if no rate
  return usd * rate;
}
...
export async function GET(request) {
  ...
  try {
    const upstream = await fetch(`${API_BASE}?${params.toString()}`, {
      ...
      next: { revalidate: 30 }, // small cache to lighten load
    });
